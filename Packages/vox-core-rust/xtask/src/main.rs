...
fn generate_bindings(workspace_root: &Path, language: Language, out_dir: &Path) -> io::Result<()> {
    ...
    ensure_success(
        Command::new(&cargo)
            ...
            .args(["build", "--locked", "-p", "vox-core-uniffi"])
            .status()?,
    ...
    ensure_success(
        Command::new(cargo)
            ...
            .args([
                ...
                OsStr::new("--no-format"),
                OsStr::new("--config"),
                workspace_root.join("uniffi-bindgen.toml").as_os_str(),
...
fn host_library_path(workspace_root: &Path) -> io::Result<PathBuf> {
    ...
    let filename = match env::consts::OS {
        "macos" => "libvox_core_uniffi.dylib",
        "linux" => "libvox_core_uniffi.so",
        "windows" => "vox_core_uniffi.dll",
        platform => {
            return Err(io::Error::new(
