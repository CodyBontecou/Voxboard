...
The governed release profile uses panic unwinding so the UniFFI facade can map a panic to the privacy-safe `InternalPanic` error without changing the process-global panic hook. Verify the release behavior with:

```sh
cargo test --release -p vox-core-uniffi panic_is_contained -- --exact
```
