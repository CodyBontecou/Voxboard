...
/// First-party Cloudflare Worker transport for privacy-safe onboarding analytics.
///
/// There is intentionally no hardcoded production endpoint in Voxboard. Release
/// builds use this transport only when `ONBOARDING_ANALYTICS_ENDPOINT_URL` is
/// supplied via build settings / Info.plist or the process environment.
public final class CloudflareOnboardingAnalyticsTransport: OnboardingAnalyticsTransport, @unchecked Sendable {
...
public final class OnboardingAnalyticsInstallIDStore: @unchecked Sendable {
    public static let defaultKey = "onboarding.analytics.install_id.v1"

    private let defaults: OnboardingAnalyticsUserDefaultsStoring
    private let key: String
    private let queue = DispatchQueue(label: "com.bontecou.voxboard.onboarding-analytics-install-id")
...
private struct OnboardingAnalyticsIngestEnvelope: Encodable {
