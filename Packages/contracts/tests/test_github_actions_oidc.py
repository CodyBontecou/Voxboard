...
class OIDCTests(unittest.TestCase):
 def setUp(self):
  ...
  self.claims={'iss':oidc.ISSUER,'aud':oidc.AUDIENCE,'repository':oidc.REPOSITORY,'repository_id':oidc.REPOSITORY_ID,'repository_owner':oidc.OWNER,'repository_owner_id':oidc.OWNER_ID,'repository_visibility':oidc.VISIBILITY,'sha':self.expected['sourceRevision'],'workflow_sha':self.expected['workflowRevision'],'workflow_ref':self.expected['workflowReference'],'run_id':'123','run_attempt':'2','ref':self.expected['ref'],'event_name':'push','runner_environment':'github-hosted','iat':self.now-1,'nbf':self.now-1,'exp':self.now+300}
 ...
 def test_repository_ref_source_workflow_issuer_and_audience_claim_mutations_fail(self):
  for claim,value in [('repository','attacker/fork'),('repository_id','1'),('ref','refs/heads/attacker'),('sha','0'*40),('workflow_sha','0'*40),('workflow_ref','CodyBontecou/vox.md/.github/workflows/other.yml@refs/heads/main'),('iss','https://attacker.invalid'),('aud','https://attacker.invalid')]:
