...
class ContractValidatorTests(unittest.TestCase):
 ...
 def test_android_phase4_new_generated_binding_bypass_source_is_rejected(self):
  validator=ROOT/"Packages/contracts/scripts/validate_toolchain.py"; p=self.root/"apps/android/app/src/release/java/md/vox/android/GeneratedBypass.java"; p.parent.mkdir(parents=True,exist_ok=True); p.write_text("final class GeneratedBypass { Object call() { return md.vox.core.Vox_core_uniffiKt.coreBuildInfo(); } }\n"); r=subprocess.run([sys.executable,str(validator),"--root",str(self.root)],cwd=self.root,text=True,capture_output=True); self.rejected(r,"production source bypasses the handwritten core bridge")
