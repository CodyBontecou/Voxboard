...
class Tests(unittest.TestCase):
 ...
 def test_case_execution_is_nonclosure_and_m3_derives_prior_cases(self):
  ...
  ids={item[0] for item in m3}; self.assertTrue({'CORE-006','CORE-007','CORE-008','CORE-009','SAF-006','SAF-007'}<=ids); self.assertNotIn('REC-004',ids); self.assertEqual(cases['REC-004']['milestone'],'M4'); self.assertNotIn('attachment',cases['SAF-001']['expected'].lower())
