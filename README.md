...
Place labels use Apple’s system reverse geocoder only when a selected field needs them. It is free and needs no API key, but it may use the network and fails softly to coordinate-only metadata. Entry prefixes, suffixes, and vault Markdown templates can use `{location}` to insert a clickable Google Maps link from the preset’s available privacy-adjusted snapshot; the token is empty when no usable opted-in location is available. Merely formatting an Apple Maps, Google Maps, or OpenStreetMap link does not contact that provider; opening a link discloses the same rounded or exact coordinates shown in the note to that provider.
