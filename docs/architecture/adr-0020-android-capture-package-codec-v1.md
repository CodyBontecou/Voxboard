...
Stale lease recovery, completed-package cleanup, reservation recovery, and journal replacement are not implemented in this slice. Composer acknowledgement, quota, SAF/provider commit, WorkManager execution, DataStore runtime, prepared artifacts, and the Rust Kotlin/native bridge are also out of scope. WorkManager and DataStore remain compile-only dependencies.
